...
certificate chain in the COSE `x5chain` header. When both are supplied, `--key`
takes precedence.
...
When `--trust-anchors` is supplied (DER or PEM; may be repeated), only those
anchors are trusted (override; no merge with system roots). When omitted, the OS
trust store is used (**not recommended** for production CoRIM trust decisions;
